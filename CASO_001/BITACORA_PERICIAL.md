...
| 026 | 2026-06-03 | Profundización obj. 6: descifrado de contraseña | Diccionario **rockyou** (14,344,391) + `descifrar_password_ken.py` (MD4/UTF-16LE) | hash NT de ken | **CONTRASEÑA DE ken = `MyPassword`** (recuperada en ~24 s). Obj. 6 cumplido al 100%. Habilita descifrado DPAPI. | Diego/Claude |
...
| 031 | 2026-06-03 | Búsqueda de usuarios ocultos (SAM + Winlogon) | `revisar_usuarios_ocultos.py` (regipy) | SAM + software | **No hay usuarios ocultos.** SAM = 5 cuentas (sin `$` ni RID extra); clave `Winlogon\SpecialAccounts\UserList` **no existe**. Confirma que el 2º SID (…-1032) NO es cuenta local. | Diego/Claude |
| 032 | 2026-06-03 | **Tarea C resuelta:** Keyword Search del 2º SID (índice ya construido) | Autopsy 4.23.1 — Keyword Search | E01 | SID `…-3933942852…` asociado a cuenta **`installuser`**, hallado como **remanente en hive SOFTWARE + SOFTWARE.LOG1 + VSS + $Unalloc** (no en ProfileList/SAM) → **Windows desplegado desde imagen maestra de otra máquina**. Salida: `29_keyword_SID_externo_installuser.csv`. Documentado §7.4/§7.12. | Equipo |
| 033 | 2026-06-03 | Fundamento jurídico del dictamen | Análisis normativo (apuntes de clase) | — | Añadida §3.2 con referencias precisas: CNPP 267/259-275/227-228/260/212/251-252/291-292/380-387; Const. 16 (párr.12-13)/20/21; CPF 211 Bis; NMX-I-27037/289; jurisprudencia SCJN (Contradicción 194/2012, 1a./J.9/2017). Encuadres legales en §7.1, §7.7, §7.10; referencias y recomendaciones ampliadas. Se retiraron citas no verificables (LFPDPPP 67-69, LGPDPPSO). | Equipo |
